...
import base64
# import requests  # Import requests for handling URL images
...
example_selection = st.selectbox('Choose an example image:', [
                                 'None'] + example_images)
...
def get_image_download_link(img, filename, text):
    ...
    href = f'<a href="data:file/txt;base64,{
        img_str}" download="{filename}">{text}</a>'
    return href
...
if example_selection != 'None':
    ...
    if img_file_buffer is not None:
        raw_bytes = np.asarray(
            bytearray(img_file_buffer.read()), dtype=np.uint8)
...
conf_threshold = st.slider("SET Confidence Threshold",
                           min_value=0.0, max_value=1.0, step=.01, value=0.5)
...
out_image, _ = process_detections(
    image, detections, conf_threshold=conf_threshold)
